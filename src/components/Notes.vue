		...
		<div style="width: 1400px; margin: 0px auto">
					...
					<div class="focus-ctrl" id="focus-ctrl">
						<ul class="focus-ctrl-wrap">
							<li
								...
								@mouseenter="changeBanner($event, index)"
							>
								<i class="item" @click="gotoLink(item)">{{ item.text }}</i>
							</li>
						</ul>
					</div>
			...
			<div>
				...
				<div
					...
					@click="getxiangqing(goods)"
					:key="index"
					style="float: left; margin: 10px 32px; cursor: pointer"
				>
			...
			<el-dialog v-model="show" title="商品详情" width="700px">
				<el-form :model="goods">
					<img
						:src="'http://localhost:8080/' + goods.imgs"
					...
					<el-form-item label="商品价格:" label-width="600"> {{ goods.price }}元/份 </el-form-item>
				...
				<div v-if="showPingjia">
					<el-table :data="pingjia" style="width: 100%">
						...
						<el-table-column label="商品评价" width="100" prop="content"> </el-table-column>
						<el-table-column label="商品时间" width="280" prop="ctime"> </el-table-column>
					</el-table>
				</div>
			</el-dialog>
		</div>
